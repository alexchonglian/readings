...
from app.forms import LoginForm, RegistrationForm, EditProfileForm, PostForm, ResetPasswordRequestForm, ResetPasswordForm, CreatePoll
from app.models import User, Post, Poll
...
def create_poll():
    ...
    if form.validate_on_submit():
        poll = Poll(
            ...
            choices=[form.choice1.data, form.choice2.data, form.choice3.data]
        )
        db.session.add(poll)
        db.session.commit()
        flask('Poll created')
        return redirect(url_for('get_poll', id=poll.id))
